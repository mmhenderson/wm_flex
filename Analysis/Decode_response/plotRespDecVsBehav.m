...
%%
if plotRespAccVsRunRTBehavZscore
    sub_colors = plasma(nSubj+1);
    zlims=[-4,4];
    for cc=1:nConds
        figure;hold all;
...
if plotRespAccVsAllRTBehav
    ...
    for cc=1:nConds
        ...
        for vv = 1:numel(resp_areas)
            ...
            for ss=1:nSubj
                rts=squeeze(rt_allsubs(ss,condlabs_allsubs(ss,:)==cc & runlabs==run & behavcorrect_allsubs(ss,:)==1));
                sconf=squeeze(respconf(ss,vv,condlabs_allsubs(ss,:)==cc));
...
% %             plot(xeval,ypred,'-','Color','k')
            ...
            axis square
            if vv>2
                xlabel('resp decoding')
            end
            ylabel('RT (s)')

            title(sprintf('%s\nrho=%.3f', ROI_names{resp_areas(vv)},rho));
        end

        suptitle(sprintf('Median RT vs resp decoding each visual area\n%s',condLabStrs{cc}))
        set(gcf,'Position',[200,200,1400,1400]);
...
if plotRespAccVsAllAccBehav
    ...
    for cc=1:nConds
        ...
        suptitle(sprintf('Behav accuracy vs resp decoding each visual area\n%s',condLabStrs{cc}))
        set(gcf,'Position',[200,200,1400,1400]);
...
%             plot([0.5,0.5],biglims,'-','Color','k');
        ...
        suptitle(sprintf('Mean behav acc vs. resp decoding in each vis area (runs) \n%s',condLabStrs{cc}))
        set(gcf,'Position',[200,200,1400,1400]);
    ...
    for cc=1:nConds
        ...
        for vv = 1:numel(resp_areas)
            ...
            for ss=1:nSubj
                ...
                x=squeeze(respconf(ss,vv,condlabs_allsubs(ss,:)==cc));
...
%             set(gca,'XTick',[0:90:360],'YTick',[0:90:360]);
            ...
            title(sprintf('%s', ROI_names{resp_areas(vv)}));
