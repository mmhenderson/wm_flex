...
for ss=1:length(sublist)
    ...
    fn2save = fullfile(save_dir,sprintf('ClassifyResponse_leavePairOut_%s_%dvox_%s.mat',class_str,nVox2Use,substr));
...
%             respLabs = mainSig(vv).RespActual;
            ...
            % taking out just trials where a response was made, and the
            % correct condition.
...
%             mainDat = mainDat - repmat(mean(mainDat,2), 1, size(mainDat, 2));
            ...
            % fold.           
            cvLabs=zeros(size(respLabs));
            for rr=1:numel(unique(respLabs))
                inds = respLabs==rr;
