...
for ss=1:length(sublist)
    ...
    fn2save = fullfile(save_dir,sprintf('ClassifyResponse_TrnDWMLoc_TRbyTR_%s_%dvox_%s.mat',class_str,nVox2Use,substr));
...
%             mainDat = mainDat - repmat(mean(mainDat,3), 1, 1, size(mainDat, 3));
            ...
            if vv==v2do(1) && cc==1
                ...
                allacc = zeros(length(ROI_names), nConds, nTRs_out);
                alld = zeros(length(ROI_names), nConds, nTRs_out);
            end
            ...
            for tr = 1:nTRs_out
                ...
                [predLabs,~] = normEucDistClass(trnDat(:,voxelindsuse),...
                    squeeze(tstDat(:,tr,voxelindsuse)),trnLabs);
