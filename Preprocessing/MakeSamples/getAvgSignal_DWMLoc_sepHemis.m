...
for ss = sublist
    ...
    fn2save = fullfile(exp_path,'Samples',sprintf('DWMLocSignalByTrial_sepHemis_%s.mat',substr));    
    ...
    %         eccVals = [];
            ...
            locSig(vv,hh).runLabs = dwm.RunLabels(trial_onset_num);
            locSig(vv,hh).trialLabs = dwm.TrialLabels(trial_onset_num);
            locSig(vv,hh).ExpDigit = dwm.ExpDigit(trial_onset_num);
            locSig(vv,hh).ActDigit = dwm.ActDigit(trial_onset_num);
            locSig(vv,hh).RespEarly = dwm.RespEarly(trial_onset_num);
            locSig(vv,hh).RT = dwm.RT(trial_onset_num);
            ...
            locSig(vv,hh).dat_avg_targ = dat_avg_targ;
            locSig(vv,hh).dat_avg_resp = dat_avg_resp;

            % this matrix is [nTrials x nTRs x nVox]
            locSig(vv,hh).dat_by_TR = dat_by_TR;
        end
    end
