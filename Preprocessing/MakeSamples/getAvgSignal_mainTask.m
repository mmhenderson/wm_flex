...
for ss = sublist
    ...
    for vv = 1:nROIs % for all visual areas I want to look at
        ...
        mainSig(vv).condLabs = main.CondLabels(trial_onset_num);
        
        targPos = main.TargPos(trial_onset_num);
        mainSig(vv).targPos = targPos;
        
        boundPos = main.BoundPos(trial_onset_num);
        mainSig(vv).boundPos = boundPos;
        
        randBoundPos=main.RandBoundPos(trial_onset_num);
        mainSig(vv).randBoundPos = randBoundPos;
               
        mainSig(vv).RespActual = main.RespActual(trial_onset_num);
        mainSig(vv).CorrectResp = main.CorrectResp(trial_onset_num);
        mainSig(vv).RTLabs = main.RTLabels(trial_onset_num);
        
        mainSig(vv).condLabStrs = {'Predictable','Random'};
        ...
        mainSig(vv).dist_to_real_bound = dist_to_real_bound;
        mainSig(vv).dir_to_real_bound = dir_to_real_bound;
        mainSig(vv).dist_to_rand_bound = dist_to_rand_bound;
        mainSig(vv).dir_to_rand_bound = dir_to_rand_bound;
        mainSig(vv).dirToBoundLabs={'CW (-)','CCW (+)'};
...
%         end
        ...
        mainSig(vv).dat_avg_resp = dat_avg_resp;
