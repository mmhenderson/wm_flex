...
plotVisConf = 1;
plotVisAcc=1;
plotVisAccOverTime=1;
plotVisConfOverTime=1;
plotVisScatterConf=1;
...
if plotVisAcc
   ...
   for cc=1:nConds
       vals = squeeze(accbins_allsubs(:,vis_inds,cc,:));       
       meanvals = squeeze(mean(vals,1));
       semvals = squeeze(std(vals,[],1)./sqrt(nSubj));
        
       plot_barsAndStars(meanvals,semvals,[],...
        [],chance_val,acclims,visual_names,distBinLabs,...
        'Accuracy',sprintf('Spatial Memory Position\n%s',condLabStrs{cc}),squeeze(col_conds(cc,:,:))')
       set(gcf,'Position',[800,800,1200,420])
   end
...
%             semvals = sem_within_allcond(cc,:);
                vals = squeeze(accbinstime_allsubs(:,vis_inds(vi),cc,bb,:));
                if nSubj==1
...
%             meanvals = meanvals_allcond(cc,:);
%             semvals = sem_within_allcond(cc,:);
                vals = squeeze(confbinstime_allsubs(:,vis_inds(vi),cc,bb,:));
                if nSubj==1
                    meanvals =squeeze(vals)';
                    semvals = [];
                else
...
if plotVisScatterConf
    ...
    for cc=1:nConds
        figure;hold all;
        npx = ceil(sqrt(numel(vis_inds)));
        npy = ceil(numel(vis_inds)/npx);
        ...
        for vv = 1:numel(vis_inds)
            ...
            for ss=1:nSubj
                ...
                y=squeeze(confall(ss,vis_inds(vv),condall(ss,:)==cc));
            ...
            % plot binned means
            vals=squeeze(confbins_allsubs(:,vis_inds(vv),cc,:));
            meanvals = squeeze(mean(vals,1));
            semvals = squeeze(std(vals,[],1)./sqrt(nSubj));
            errorbar(dist_bin_centers,meanvals,semvals,'Color','k')
...
%             set(gca,'XLim',[0,90],'YLim',conflims);
        ...
        suptitle(sprintf('Spatial decoding in each area vs. target distance from bound\n%s',condLabStrs{cc}))
