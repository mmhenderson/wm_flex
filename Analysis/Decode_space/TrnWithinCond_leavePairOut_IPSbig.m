...
for ss=1:length(sublist)
    ...
    fn2save = fullfile(save_dir,sprintf('TrnWithinCond_leavePairOut_IPSbig_%s_max%dvox_%s.mat',class_str, nVox2Use,substr));
    ...
    for vv = 1:length(ROI_names)
        ...
        for cc=1:nConds
            ...
            conf_this_cond = nan(numel(posLabs),1);
            ...
            if ~isempty(nVox2Use) && nVox2Use<size(allDat,2)
                fprintf('running voxel selection f-test for %s %s: %s condition\n',substr, ROI_names{vv},condLabStrs{cc})
                voxStatTable = zeros(size(allDat,2),nCV);
                for rr = 1:nCV
                    inds = cvLabs~=rr;
                    pvals = zeros(size(allDat,2), 1);
                    dat = allDat(inds,:);
                    lab = binLabs(inds,:);
                    parfor vx = 1:size(allDat,2)
                         % choose the voxels        
            ...
            for gg=1:nGroups
                ...
                dat_this_group = squeeze(allDat(inds2use, :));
                ...
                allacc(vv,cc,gg) = mean(thesepredlabs==group_labs);
                alld(vv,cc,gg) = get_dprime(thesepredlabs,group_labs,unique(group_labs));
