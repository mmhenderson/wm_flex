...
for ss=1:length(sublist)
    ...
    fn2save = fullfile(save_dir,sprintf('TrnSWMLoc_TestWM_TRbyTR_%s.mat',substr));
...
%         tstLabsIEM = trnLabsIEM;
        ...
        tstDatIEM = mainSig(vv).dat_by_TR;
        nTRs_out = size(tstDatIEM,2);
        % trying to get rid of baseline shifts here. 
        % Averaging across all voxels, at each TR on each trial.
        tstDatIEM = tstDatIEM - repmat(mean(tstDatIEM,3), 1, 1,size(tstDatIEM,3));
%         
        ...
        chan_resp = NaN(size(tstDatIEM,1), length(xx), nTRs_out);   
        ...
        for b = 1:(n_deg/n_spat_chans)
            ...
            for tr = 1:nTRs_out
                curdat = squeeze(tstDatIEM(:,tr,:));
                chan_resp(:,chan_center,tr) = ((w*w')\w*curdat').';
            end
        ...
        for t = 1:size(chan_resp,1)
            for tr =1:nTRs_out
                ...
                chan_resp_shift(t,:,tr) =  wshift('1D', rec,ceil(tstLabsIEM(t)-shift_to));
            end
        end
