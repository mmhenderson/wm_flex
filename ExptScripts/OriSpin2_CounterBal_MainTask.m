function trialInfo = OriSpin2_CounterBal_MainTask(subNum,makePlots)
...
% within a run, task conditions are interleaved.

% within session, counterbalanced for
    % boundary line position 
    % distribution of memory stimuli (positions)
    
% across two sessions, counter-balanced for response mapping.
    ...
    % if un-predictable trial, what is the "random" boundary that gets
    % previewed?
    rand_bound_position = zeros(nSess, nBlocksSess, nTrialsBlock);
    ...
    for task = 1:nTasks
        ...
        if task==2
            % for un-predictable condition, make a random order to use for
            % the uninformative "preview" disk.
            rand_bound_pos_this_task = reshape(bound_pos_this_task(randperm(numel(bound_pos_this_task))), size(bound_pos_this_task));
            rand_bound_position(taskinds_rand==task) = rand_bound_pos_this_task;
        else
            rand_bound_position(taskinds_rand==task) = NaN;
        end
    end
    ...
    trialInfo.RandBoundPosition = rand_bound_position;
    ...
    for se = 1:nSess
        for rr = 1:nBlocksSess
            for tt = 1:nTrialsBlock
                ...
                % also checking here to make sure the random boundary is
                % assigned only for trials in unpredictable condition.
                if trialInfo.Task(se,rr,tt)==1
                    assert(isnan(trialInfo.RandBoundPosition(se,rr,tt)));
                else
                    assert(~isnan(trialInfo.RandBoundPosition(se,rr,tt)));
                end
            end
