...
plotVisMotor = 1;
% plotVisMotorSS = 1;
areas2avg = {[1:5],[6:9],[12:14]};

avg_names = {'early','parietal','sensorimotor'};
...
allsub_HRFs_mean = zeros(nSubj, length(areas2avg), nConds, nTRs_out);
allsub_HRFs_sem = zeros(nSubj, length(areas2avg), nConds, nTRs_out);
...
for ss=1:length(sublist)
    ...
    %%
    for aa = 1:numel(areas2avg) % for all visual areas I want to look at
        
        % for this "average" area - start by concatenating voxels across
        % the sub-areas (e.g. parts of IPS)
        for vi = 1:numel(areas2avg{aa}) 
        ...
        for cc=1:nConds
            allsub_HRFs_mean(ss,aa,cc,:) = mean(maintaskHRFs(:,cc,:),3);
            allsub_HRFs_sem(ss,aa,cc,:) = std(maintaskHRFs(:,cc,:),[],3)./sqrt(nVox);
        end
...
real_sr_stat = nan(numel(areas2avg),nTRs_out);
rand_sr_stat = nan(numel(areas2avg), nTRs_out, nPermIter);

% p_diff_sr=nan(nROIs,nTRs_out);
for aa=1:numel(areas2avg)
    for tr=1:nTRs_out
        realvals = squeeze(vals(:,aa,:,tr));
        % what is the sign-rank statistic for the real data?
        real_sr_stat(aa,tr) = signrank_MMH(realvals(:,1),realvals(:,2));
        ...
        parfor ii=1:nPermIter          
            ...
            rand_sr_stat(aa,tr,ii) = signrank_MMH(randvals(:,1),randvals(:,2));
        end
    end
end
...
array2table(squeeze(p_cond_diff(:,1:15)),...
    'RowNames',avg_names,'VariableNames',strseq('CondDiff_TR',1:15))
array2table(squeeze(p_cond_diff(:,16:30)),...
    'RowNames',avg_names,'VariableNames',strseq('CondDiff_TR',16:30))

%%
if plotVisMotor
    nplotsx = ceil(sqrt(numel(areas2avg)));
    figure;hold all;
      
    for aa = 1:numel(areas2avg)
        subplot(nplotsx,ceil(numel(areas2avg)/nplotsx),aa);hold all;
        lh=[];lx=0;ll=[];
        for cc=1:nConds
          
            % take contra - ipsi difference
            vals = allsub_HRFs_mean(:,aa,cc,:);

            if nSubj==1
                meanvals =squeeze(vals);
                semvals = squeeze(allsub_HRFs_sem(:,aa,cc,:));
            else
...
%                 lh=[lh, errorbar(tax, meanvals, semvals,'Color',col_conds(cc,:,bb),'LineWidth',lw)];
        ...
        for alph=1:numel(alpha_vals)
            inds2plot=p_cond_diff(aa,:)<alpha_vals(alph);
            plot(tax(inds2plot), repmat(sig_heights(nConds+1),sum(inds2plot),1),'.','Color',diff_col,'MarkerSize',alpha_ms(alph));
            lh=[lh, plot(tax(1)-5, sig_heights(nConds+1),'.','Color',diff_col,'MarkerSize',alpha_ms(alph))];
            ll{numel(ll)+1} = sprintf('p<%.03f',alpha_vals(alph));
        end
                
            
        set(gca, 'FontSize', 12, 'XLim',[0 max(tax)],'YLim',ylims,'XTick',[0,10,20],'YTick',[0, 0.5])
        ...
        if aa==numel(areas2avg)
            legend(lh, ll);
        end

        
        title(sprintf('%s',avg_names{aa}));
...
%     saveas(gcf,fullfile(figpath,'Deconv_sepconds_allareas.pdf'),'pdf');
