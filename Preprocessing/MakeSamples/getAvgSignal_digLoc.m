...
for ss = sublist
    ...
    fn2save = fullfile(exp_path,'Samples',sprintf('DigLocSignalByTrial_%s.mat',substr));    
    ...
    for vv = 1:nROIs % for all visual areas I want to look at
        ...
        locSig(vv).runLabs = digLoc.RunLabels(trial_onset_num);
        locSig(vv).trialLabs = digLoc.TrialLabels(trial_onset_num);
        locSig(vv).ExpDigit = digLoc.ExpDigit(trial_onset_num);
        locSig(vv).ActDigit = digLoc.ActDigit(trial_onset_num);
