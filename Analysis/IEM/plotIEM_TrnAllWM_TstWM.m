...
% training and testing on trials from both task conditions together (rather
% than within each condition). 
%%
...
for ss=1:nSubj
    ...
    fn = fullfile(exp_path,'Analysis','IEM','IEM_results',sprintf('TrnAllWM_TstWM_%s.mat',substr));
    load(fn);
...
%% pairwise condition comparisons - paired t-test.
numcores = 8;
if isempty(gcp('nocreate'))
    parpool(numcores);
end
