...
for ss=1:length(sublist)
    ...
    fn2save = fullfile(save_dir,sprintf('ClassifyActualResponse_TRbyTR_%s_%dvox_%s.mat',class_str,nVox2Use,substr));
...
%             mainDat = mainDat - repmat(mean(mainDat,3), 1, 1, size(mainDat, 3));
            ...
            if vv==v2do(1) && cc==1
                ...
                nTrialsTotal=size(respLabs,1);
                allacc = nan(length(ROI_names), nConds, nTRs_out);
                alld = nan(length(ROI_names), nConds,  nTRs_out);
                allconf = nan(length(ROI_names),nConds, nTrialsTotal, nTRs_out);
    ...
    %         for tr = [4]
            for tr = 1:nTRs_out
                ...
                [~,~,predLabs,normEucDist] = my_classifier_cross_wconf(trnDat,trnLabs,...
                    ...
                    tstCV,class_str,1000,nVox2Use_now,voxStatTable,1);
                ...
                % note this condition doesn't always hold if there is
                % re-sampling, because the way re-sampling iterations are
                % combined to get accuracy might give a different answer than
                % the mean confidence estimates across balancing iterations.
