...
% Screen('Preference', 'SkipFixColorSyncTests', 1);
...
try
    ...
    if p.TrainingSubject==1
        p.fnsave = [datadir_local 'OriSpin2_TRAINING_S', p.SubNumStr, '_' t.TheDate '_MainTask.mat'];
    else
        p.fnsave = [datadir_local 'OriSpin2_S', p.SubNumStr, '_' t.TheDate '_MainTask.mat'];
    end
    
    if exist(p.fnsave,'file')
        load(p.fnsave);
    else            
        trialInfo = OriSpin2_CounterBal_FMRI(p.SubNum,0);
    end
    ...
    t.ITI = trialInfo.ITI(1,p.RunNumGlobal,:);
