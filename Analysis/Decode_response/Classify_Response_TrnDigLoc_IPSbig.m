...
for ss=1:length(sublist)
    ...
    fn2load = fullfile(exp_path,'Samples',sprintf('MainTaskSignalByTrial_IPSbig_%s.mat',substr));
    load(fn2load);
    fn2load = fullfile(exp_path,'Samples',sprintf('DigLocSignalByTrial_IPSbig_%s.mat',substr));
    load(fn2load);
    save_dir = fullfile(curr_dir,'Decoding_results');
    if ~isfolder(save_dir)
        mkdir(save_dir);
    end
    fn2save = fullfile(save_dir,sprintf('ClassifyResponse_TrnDigLoc_IPSbig_%s_%dvox_%s.mat',class_str,nVox2Use,substr));
...
%             trials2use = locSig(vv).ActDigit == locSig(vv).ExpDigit;
            ...
            %%
%             avg_main_cond1 = mean(mainDat(tstLabs==1,:),1);
%             avg_main_cond2 = mean(mainDat(tstLabs==2,:),1);
%             
%             avg_dig_cond1 = mean(trnDat(trnLabs==1,:),1);
%             avg_dig_cond2 = mean(trnDat(trnLabs==2,:),1);
%             
%             figure;hold all;
%             scatter(avg_main_cond1, avg_main_cond2);
%             scatter(dig_main_cond1, dig_main_cond2);
%             
%             figure;hold all;
%             scatter(avg_main_cond1, avg_dig_cond1);
%             scatter(avg_main_cond2, avg_dig_cond2);
%             
            %%
