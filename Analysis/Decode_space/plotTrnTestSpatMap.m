...
% trained and tested within conditions of main task.
% Decoding analysis itself performed in TrnWithinCond_leavePairOut.m and 
% saved as mat file. This script loads that file, does all stats and plotting. 
%%
clear
close all;
...
for ss=1:length(sublist)
    ...
    fn2load = fullfile(save_dir,sprintf('TrnTestSpatMap_%s_max%dvox_%s.mat',class_str,nVox2Use,substr));
    load(fn2load);
    
    % averaging decoding performance over the four decoding schemes (0 vs
    % 180, 45 versus 225, etc)
    acc_allsubs(ss,:,:) = mean(squeeze(allacc(plot_order_all,:)),2);
    d_allsubs(ss,:,:) = mean(squeeze(alld(plot_order_all,:)),2);
    
    accrand_allsubs(ss,:,:,:) = mean(squeeze(allacc_rand(plot_order_all,:,:)),2);
...
array2table([p_sr(vismotor_inds,1)],...
    'RowNames',vismotor_names,'VariableNames',{'pval_signrank'})
...
array2table(sum(is_sig_ss(:,vismotor_inds),1)','RowNames',vismotor_names,'VariableNames',condLabStrs)
...
%     
    ...
    b(end).BarWidth=bw;
    leg=legend(lh,{'SpatMap','p<0.05','0<0.01','p<0.001'},'Location','EastOutside');
%     uistack(b(end),'top');
%     uistack(b(end-1),'top')
    set(gcf,'color','white')
